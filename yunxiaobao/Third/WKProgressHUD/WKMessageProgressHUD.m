...
- (void)compositeElements {
    self.backView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(self.textLabel.frame) + 30, CGRectGetHeight(self.textLabel.frame) + 30)];
    self.backView.center = CGPointMake(CGRectGetMidX(self.selfView.frame), CGRectGetMidY(self.selfView.frame));
    self.backView.backgroundColor = [UIColor colorWithWhite:0.3 alpha:0.7];
    self.backView.layer.cornerRadius = 10;
    
    self.textLabel.center = CGPointMake(CGRectGetMidX(self.backView.bounds), CGRectGetMidY(self.backView.bounds));
//    self.textLabel.font = [UIFont systemFontOfSize:16*LXWidth/320];
//    [DQLabel Label:self.textLabel smallFont:14];
    self.textLabel.font = [UIFont systemFontOfSize:(DelouchWidth / 375) * 14];
