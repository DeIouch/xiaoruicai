...
+ (instancetype)popMessage:(NSString *)text inView:(UIView *)view duration:(NSTimeInterval)duration animated:(BOOL)animated {
    UIView *backView = [[UIView alloc]initWithFrame:CGRectMake(0, (DelouchHeight - (DelouchWidth / 375) * 15) / 2, view.frame.size.width, (DelouchWidth / 375) * 15)];
    backView.backgroundColor = [UIColor clearColor];
    [view addSubview:backView];
    WKProgressHUD *HUD = [[WKMessageProgressHUD alloc] initWithFrame:backView.bounds];
